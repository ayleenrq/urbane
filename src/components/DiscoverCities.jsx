            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div ref={leftRef}>
                    <h2 className="text-4xl md:text-5xl font-semibold leading-tight mb-8">
                        ...
                        <span className="inline-flex items-center justify-center w-16 h-10 bg-gray-100 dark:bg-gray-800 rounded-full align-middle mx-1">
                            <img
                                alt="city icon"
                    ...
                    <div className="flex flex-wrap gap-3 mb-8">
                        {["Cleanliness", "Honest", "Trusted", "Luxury"].map((tag) => (
                            <span key={tag} className="px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-800 text-sm flex items-center cursor-default select-none hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                                <span className="w-1.5 h-1.5 rounded-full bg-black dark:bg-white mr-2" />
                                {tag}
                            </span>
                        ))}
                    </div>
                    ...
                    {/* Banner */}
                    <div className="bg-surface-light dark:bg-surface-dark py-3 px-4 rounded-full flex items-center justify-between gap-4 mb-8">
                        <div className="flex items-center gap-3">
                            <span className="w-9 h-9 flex-shrink-0 bg-white dark:bg-black rounded-full flex items-center justify-center shadow-sm">
                        ...
                        <button
                            className="flex-shrink-0 bg-white dark:bg-black pl-4 pr-3 py-2 rounded-full text-xs font-bold flex items-center gap-2 shadow-sm hover:shadow-md transition-shadow active:scale-95 whitespace-nowrap"
                            onClick={() => alert("Showing full details…")}
                        >
                            Show More
                            <span className="w-5 h-5 flex items-center justify-center bg-black dark:bg-white text-white dark:text-black rounded-full">
                ...
                <div ref={rightRef} className="relative h-[600px] rounded-3xl overflow-hidden group">
                    ...
                    />
                    <div className="absolute bottom-6 left-6 right-6 bg-white dark:bg-black/90 backdrop-blur-sm p-6 rounded-3xl shadow-xl">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <button
                                ...
                                className={`w-10 h-10 flex items-center justify-center border rounded-full transition-all hover:scale-110 active:scale-95 ${saved ? "border-black bg-black text-white" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"
                                    }`}
...
const FaqCard = ({ item }) => {
    ...
    return (
        <button
            onClick={() => setOpen((o) => !o)}
            className="w-full h-full bg-surface-light dark:bg-surface-dark p-6 rounded-3xl text-left hover:shadow-md transition-shadow duration-300 group flex flex-col"
        >
            <div className="w-8 h-8 flex-shrink-0 rounded-full border border-gray-300 dark:border-gray-600 flex items-center justify-center mb-4 text-gray-500 text-sm font-medium group-hover:border-black group-hover:text-black dark:group-hover:border-white dark:group-hover:text-white transition-colors">
                {open ? "–" : "?"}
            </div>
